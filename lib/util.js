...
_.mixin(/** @lends util */ {
    ...
    mergeDefined: function (target, source) {
        ...
        for (key in source) {
            source.hasOwnProperty(key) && !_.isUndefined(source[key]) && (target[key] = source[key]);
        }
...
util = {
    ...
    bufferOrArrayBufferToString: function (buffer, charset) {
        ...
        // Bail out immediately if it is an array buffer.
        if (buffer.toString(UTF8) === '[object ArrayBuffer]') {
            return util.arrayBufferToString(buffer);
        }

        try {
            str = buffer.toString(charset);
        }
        catch (e) {
            // Falling back to iconvlite to decode
            if (iconvlite.encodingExists(charset)) {
                str = iconvlite.decode(buffer, charset);
            }
            else {
                str = buffer.toString(UTF8);  // Default to utf8 if iconvlite also not supporting the charset
            }
        }
        return str;
    },
