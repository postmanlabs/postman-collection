...
var _ = require('lodash').noConflict(),
     ...
     * @type {string}
     */
    DEFAULT_JSON_FILENAME = 'response',
...
util = {
    ...
    /**
     * Parse JSON after trimming and removing comments and BOM
     *
     ...
     * @returns {Object}
     */
    parseJSON: function (jsonString, reviver, strict) {
        // clean json if possible
        if (!strict && (jsonString != null) && _.isFunction(jsonString.toString)) {
            jsonString = jsonString.toString(); // get the string variant
            ...
            else if ((jsonString.charCodeAt(0) === 0xFE) && (jsonString.charCodeAt(1) === 0xFF)) {
                jsonString = jsonString.slice(2);
            }
            // little endian UTF-16 BOM
            else if ((jsonString.charCodeAt(0) === 0xFF) && (jsonString.charCodeAt(1) === 0xFE)) {
                jsonString = jsonString.slice(2);
            }
            // UTF-8 BOM
            else if ((jsonString.charCodeAt(0) === 0xEF) && (jsonString.charCodeAt(1) === 0xBB) &&
                (jsonString.charCodeAt(2) === 0xBF)) {
        ...
        // pass it via parse-json module for more detailed error
        return parseJSON(jsonString, reviver, DEFAULT_JSON_FILENAME);
    }
};
