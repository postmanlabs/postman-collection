var faker = require('@faker-js/faker/locale/en').faker,
    ...
    // faker no longer ships a color name generator (`commerce.color` was removed in v9).
    // ref: https://github.com/faker-js/faker/blob/v5.5.3/src/locales/en/commerce/color.ts
    COLORS = ['red', 'green', 'blue', 'yellow', 'purple', 'mint green', 'teal', 'white', 'black', 'orange', 'pink',
    ...
    // faker no longer ships a company suffix generator (`company.companySuffix` was removed in v9).
    // ref: https://github.com/faker-js/faker/blob/v5.5.3/src/locales/en/company/suffix.ts
    COMPANY_SUFFIXES = ['Inc', 'and Sons', 'LLC', 'Group'],

    // faker's image generators no longer support categories (`image.imageUrl` and the category methods were
    // removed in v9) and `image.urlLoremFlickr` is deprecated since v10.1.0, so these URLs are built locally.
    IMAGE_BASE_URL = 'https://loremflickr.com/640/480',

    // phone number in the `###-###-####` format, replaces faker's removed `phone.phoneNumberFormat`
    PHONE_NUMBER_PATTERN = '[0-9]{3}-[0-9]{3}-[0-9]{4}',

    /**
     * Returns a generator for a random image URL of the given category.
     *
     * @private
     * @param {String} [category] - image category, omit for an uncategorized image
     * @returns {Function}
     */
    imageUrl = function (category) {
        var url = category ? IMAGE_BASE_URL + '/' + category : IMAGE_BASE_URL;
    ...
    dynamicGenerators = {
        ...
        // faker.phone.number returns phone number in random formats with or
        // without extension. this only returns a phone number without extension.
        $randomPhoneNumber: {
            description: 'A random 10-digit phone number',
            generator: function () {
                return faker.helpers.fromRegExp(PHONE_NUMBER_PATTERN);
            }
        },

        // faker.phone.number returns phone number in random formats with or
        // without extension. this only returns a phone number with extension.
        $randomPhoneNumberExt: {
            description: 'A random phone number with extension (12 digits)',
            generator: function () {
                return faker.number.int({ min: 1, max: 99 }) + '-' + faker.helpers.fromRegExp(PHONE_NUMBER_PATTERN);
            }
        },
        ...
        // fakers' word.words returns a fixed number of words.
        ...
        // faker's location.latitude and location.longitude return numbers.
        // these return coordinates with a fixed precision of 4 decimal places.
        $randomLatitude: {
            description: 'A random latitude coordinate',
            generator: function () {
                return faker.location.latitude().toFixed(4);
            }
        },
        $randomLongitude: {
            description: 'A random longitude coordinate',
            generator: function () {
                return faker.location.longitude().toFixed(4);
            }
        },

        // faker no longer generates color names, this returns one from a local list
        $randomColor: {
        ...
        // faker no longer generates company suffixes, this returns one from a local list
        $randomCompanySuffix: {
            description: 'A random company suffix (e.g. Inc, LLC, Group)',
            generator: function () {
                return faker.helpers.arrayElement(COMPANY_SUFFIXES);
            }
        },
        $randomCatchPhrase: {
        ...
        // faker no longer generates masked numbers, this returns the last 4 digits
        $randomCreditCardMask: {
            description: 'A random masked credit card number',
            generator: function () {
                return faker.string.numeric({ length: 4, allowLeadingZeros: true });
            }
        },
        $randomPrice: {
        ...
        // faker's internet.url always uses https and appends a slash randomly
        $randomUrl: {
            description: 'A random URL',
            generator: function () {
                return faker.internet.url({
                    appendSlash: false,
                    protocol: faker.datatype.boolean() ? 'http' : 'https'
                });
            }
        },
        $randomDomainName: {
        ...
        // faker's internet.ip returns either an IPv4 or an IPv6 address randomly
        $randomIP: {
        ...
        // faker's string.alphanumeric returns mixed case characters
        $randomAlphaNumeric: {
            description: 'A random alpha-numeric character',
            generator: function () {
                return faker.string.alphanumeric({ casing: 'lower' });
            }
        },
