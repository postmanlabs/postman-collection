 ...
 */
Substituter = function (variables) {
    this.variables = variables;
};

_.extend(Substituter.prototype, /** @lends Substituter.prototype */ {
    /**
     * Find a key from the array of variable objects provided to the substituter
     *
     * @param {String} key
     * @returns {*}
     */
    ...
    parse: function (value) {
        ...
        // tokens
        var replacer = Substituter.replacer(this);

        // replace the value once and keep on doing it until all tokens are replaced or we have reached a limit of
        // replacements
        do {
            value = value.replace(Substituter.REGEX_EXTRACT_VARS, replacer);
        } while (value.replacements && (value.substitutions < Substituter.VARS_SUBREPLACE_LIMIT));

        // do a final check that if recursion limits are reached then replace with blank string
        if (value.substitutions >= Substituter.VARS_SUBREPLACE_LIMIT) {
            value = value.replace(Substituter.REGEX_EXTRACT_VARS, E);
        }
...
_.extend(Substituter, /** @lends Substituter */ {
    /**
    ...
    /**
     * Defines the number of times the variable substitutionmechanism will repeat until all tokens are resolved
     * @type {Number}
     */
    VARS_SUBREPLACE_LIMIT: 21,
    /**
    ...
    /**
     * Create an instance of a substituter or reuse one
     *
     * @param {Array|Substituter} variables
     * @returns {Substituter}
     */
    box: function (variables) {
        return (variables instanceof Substituter) ? variables : new Substituter(variables);
    },

    /**
     * Checks whether a variable is instance of substituter
     *
     * @param {*} subject
     * @returns {Boolean}
     */
    isInstance: function (subject) {
        return (subject instanceof Substituter);
    },
     ...
     * @private
     * @param {Substituter} substituter
     * @returns {Function}
     */
    replacer: function (substituter) {
        return function (match, token) {
            var r = substituter.find(token);
            r && _.isFunction(r) && (r = r());
            r && _.isFunction(r.toString) && (r = r.toString());
            return Substituter.NATIVETYPES[(typeof r)] ? r : match;
        };
    }
});
