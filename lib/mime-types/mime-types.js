var util = require('../util'),
    ...
    /**
     * Enum for characterSet
     * @private
     * @readonly
     * @enum {string} characterSetEnum
     */
    characterSetEnum = {
        UTF8: 'utf8',
        utf8: 'utf-8',
    ...
    */
     ...
     * @private
     */
    characterDecodeHashMap = {
        [characterSetEnum.iso88591]: function (binary) {
            return getLatin(binary);
        },
        [characterSetEnum.utf8]: function (binary) {
            if (!supportsBuffer) {
                return;
            }
            return (Buffer.from(binary, BINARY)).toString(characterSetEnum.UTF8);
        }
    },
    ...
    splitFileName = function (fileName) {
        ...
        // Returns default filename if filename is empty in dispositionHeader
...
module.exports = {
    ...
    * is returned if disposition header doesn't  have  filename or its unparsable
    *
    * @param {String} dispositionHeader -Content-disposition Header from the response
    * @param {String} normalisedCharType
    * @returns {Object} Returns file base name and extension name
    */
    filename: function (dispositionHeader, normalisedCharType) {
        var parsedFileName = getFileNameFromDispositionHeader(dispositionHeader);

        // Takes default filename, if its not present in disposition header or unparsable
        parsedFileName = parsedFileName || {
            name: DEFAULT_RESPONSE_FILENAME,
            ext: mimeType.extension(normalisedCharType) || E
        };
