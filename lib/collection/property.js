var _ = require('../util').lodash,
    ...
    ITERATION_LOCKED = Symbol('Substitution Interation Lock'),
    DISABLED = 'disabled',
...
_.assign(Property, /** @lends Property */ {
     ...
     *
     * @example
     * Proprty.findSubstitutions({request: 'https://{{host}}/{{path1}}-action/'});
     * -> ['host', 'path1']
     *
     * @todo
     * - tonne of scope for performance optimisations
    ...
    findSubstitutions: function (obj) {
        let substitutionKeys = [],
            regex = Substitutor.REGEX_EXTRACT_VARS;

        // since this function is recursive, we double check to avoid any cyclic error
        if (_.isNil(obj) || (obj[ITERATION_LOCKED] === true)) {
            return substitutionKeys;
        }

        // we recurse for arrays, then we recurse for objects and finally when we land on a string,
        // we extract substitutions using regex
        if (_.isArray(obj)) {
            obj[ITERATION_LOCKED] = true;
            for (let i = 0, ii = obj.length; i < ii; i += 1) {
                substitutionKeys = substitutionKeys.concat(Property.findSubstitutions(obj[i]));
            }
            delete obj[ITERATION_LOCKED];
        }
            ...
            for (const key in obj) {
                if (_.has(obj, key)) {
                    substitutionKeys = substitutionKeys.concat(Property.findSubstitutions(obj[key]));
                }
            }
            delete obj[ITERATION_LOCKED];
        }
            ...
            let matches = obj.match(regex);

            if (matches && matches.length) {
                for (let i = 0, ii = matches.length; i < ii; i += 1) {
                    substitutionKeys.push(matches[i].slice(2, -2)); // remove the brace prefix and suffix
                }
            }
        }
