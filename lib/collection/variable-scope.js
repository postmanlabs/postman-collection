var _ = require('../util').lodash,
    ...
    ChangeTracker = require('./change-tracker').ChangeTracker,
...
_.inherit((
    ...
    VariableScope = function PostmanVariableScope (definition, layers, options) {
        ...
        if (definition && definition.changes) {
            this.changes = new ChangeTracker(definition.changes);
        }

        if (options && options.enableTracking) {
            this._postman_enableTracking = true;
            this.changes = new ChangeTracker(definition && definition.changes, options.changeTracker);
        }
    }), Property);
...
_.assign(VariableScope.prototype, /** @lends VariableScope.prototype */ {
    ...
    set: function (key, value, type) {
        ...
        if (variable) {
            ...
            this._postman_enableTracking && this.changes.track('updated', update);
            return;
        }

        this.values.add(update);
        this._postman_enableTracking && this.changes.track('added', update);
    },
    ...
    unset: function (key) {
        ...
        this.changes.track('removed', { key: key });
    },
    ...
    clear: function () {
        if (this._postman_enableTracking) {
            ...
            this.values.each(function (variable) {
                this.changes.track('removed', { key: variable.key });
            }.bind(this));
        }
