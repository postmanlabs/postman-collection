var _ = require('../util').lodash,
    ...
    /**
     * Fetches a variable from the given VariableList if present.
     * Also, normalize variable reference in the list if it's disabled.
     *
     * @param {VariableList} variableList - Variable list to search in
     * @param {String} variableName - Variable name to search for
     * @returns {Variable|undefined}
     */
    getNormalizedVariable = function (variableList, variableName) {
        var variable = variableList.one(variableName);

        if (variable && variable.disabled) {
            // normalize variable reference if its disabled
            variable = variableList.normalizeReference(variable);
        }
...
_.assign(VariableScope.prototype, /** @lends VariableScope.prototype */ {
    ...
    has: function (variableName) {
        var variable = getNormalizedVariable(this.values, variableName);
     ...
     */
    get: function (key) {
        var variable = getNormalizedVariable(this.values, key),
            i,
            ii;
        ...
        if (!(variable || !this._layers)) {
            for (i = 0, ii = this._layers.length; i < ii; i++) {
                variable = getNormalizedVariable(this._layers[i], key);
    ...
    unset: function (key) {
        ...
        // early bail out if there's no variable with the given name
        if (!list.one(key)) {
            return;
        }

        // remove all enabled members from the list
        _.remove(list.members, function (variable) {
            // bail out if variable name didn't match
            if (variable.key !== key) {
                return false;
            }

            // don't delete disabled variables, change the reference in the list
            if (variable.disabled) {
                list.reference[key] = variable;
                return false;
            }

            delete variable[__PARENT]; // unlink from its parent
            return true;
        });

        // delete entire reference if there's no disabled variable assigned
        if (list.reference[key] && !list.reference[key].disabled) {
            delete list.reference[key];
        }
