...
_.assign(VariableScope.prototype, /** @lends VariableScope.prototype */ {
    ...
    has: function (variableName) {
        var match = _.isString(variableName) && this.values.lastEnabled(variableName);
     ...
     */
    get: function (key) {
        var variable = this.values.lastEnabled(key),
            i,
            ii;
        ...
        if (!(variable || !this._layers)) {
            for (i = 0, ii = this._layers.length; i < ii; i++) {
                variable = this._layers[i].lastEnabled(key);
                if (variable) { break; }
            }
        }
     ...
     */
    unset: function (key) {
        this.values.removeLastEnabled(key);
