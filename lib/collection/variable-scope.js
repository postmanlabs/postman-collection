 ...
 * | unset  | delete {Variable} | noop            | delete last enabled {Variable} also,                               |
 * |        |                   |                 | update the reference with last enabled in rest of the list         |
 */
_.assign(VariableScope.prototype, /** @lends VariableScope.prototype */ {
    ...
    has: function (variableName) {
        var variable = this.values.one(variableName);


        if (variable && variable.disabled) {
            // normalize variable reference if its disabled
            variable = this.normalizeReference(variable);
        }
    ...
    get: function (key) {
        ...
        if (!(variable || !this._layers)) {
            for (i = 0, ii = this._layers.length; i < ii; i++) {
                ...
                if (variable && variable.disabled) {
                    // normalize variable reference in layer if its disabled
                    variable = this.normalizeReference(variable, this._layers[i]);
                }
    ...
    unset: function (key) {
        var i,
            member,
            newReference,
            variable = this.values.one(key);

        // bail out if variable doesn't exists
        if (!variable) {
            return;
        }
        ...
        // bail out if variable is disabled.
        // @note disabled variables are immutable.
        if (variable.disabled) {
            return;
        }

        // traverse the members list in reverse direction in order to find the
        // last enabled variable.
        for (i = this.values.members.length - 1; i >= 0; i--) {
            member = this.values.members[i];
            if (member === variable) {
                // remove variable from VariableList's members
                this.values.members.splice(i, 1);
                break;
            }
        }

        // update the reference if there's any duplicate left in the VariableList
        newReference = this.normalizeReference(variable);

        // if newReference is same as initial variable, delete its reference
        if (newReference === variable) {
            delete this.values.reference[variable.key];
        }
    ...
    /**
     * Normalize variable reference in current scopes VariableList or in given
     * layer's VariableList. This is used to fix the disabled variable reference
     * with its last enabled duplicate.
     *
     * @note Due to optimization reasons few safety checks are avoided as its an
     * private method. Make sure valid instance of Variable(disabled) & VariableList are passed.
     *
     * @private
     * @param {Variable} variable - Variable whose reference needs to be updated
     * @param {VariableList} [list] - VariableList in which the reference to be updated
     * @returns {Variable} - In case of duplicates, returns last enabled otherwise initial variable
     */
    normalizeReference: function (variable, list) {
        list = list || this.values; // defaults to current scopes VariableList

        var i,
            newVar,
            index = list._postman_listIndexKey; // `key` for Variable

        // traverse the members list in reverse direction in order to find the last enabled
        for (i = list.members.length - 1; i >= 0; i--) {
            newVar = list.members[i];
            if (newVar[index] === variable[index] && !newVar.disabled) {
                // update the variable reference if newVar is not disabled.
                list.reference[variable[index]] = newVar;

                // return updated reference variable
                return newVar;
            }
        }

        // return initial variable if there's no enabled duplicate is present in
        // the members list.
        return variable;
    }
});
