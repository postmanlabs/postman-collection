...
_.assign(VariableList.prototype, /** @lends VariableList.prototype */ {
    ...
    syncFromObject: function (obj, track) {
        ...
        _.forOwn(obj, function (value, key) {
            tmp = _.isObject(value) ? value : { value: value };

            // we need to create new variable if exists or update existing
            if (list.has(key)) {
                list.one(key).update(tmp);
        ...
        _.forEach(_.keys(list.reference), function (key) {
            if (obj.hasOwnProperty(key)) { return; } // do not delete if source obj has this variable
    ...
    env: function (index, assign, clear) {
        ...
        return arguments.length ? (env && PropertyBase.toJSON(env)) : _.map(this._environments, PropertyBase.toJSON);
    }
});
