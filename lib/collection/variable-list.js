...
_.assign(VariableList.prototype, /** @lends VariableList.prototype */ {
    ...
    /**
     * Normalize disabled variable reference in VariableList.
     * This updates the disabled variable `reference` in VariableList with its
     * last enabled duplicate(if found) in the `members` list.
     *
     * @note Due to optimization reasons, few safety checks are omitted as well as
     * its a private method so, make sure valid instance of `Variable`(disabled)
     * is used.
     *
     * @private
    ...
    normalizeReference: function (variable) {
        ...
        // return initial variable if there's no enabled duplicate is present
        return variable;
    }
});
