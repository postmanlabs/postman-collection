...
_.inherit((
    ...
    RequestBody = function PostmanRequestBody (options) {
        ...
        options && this.update(options); // only set the request body if options are truthy
    }), PropertyBase);
...
_.assign(RequestBody.prototype, /** @lends RequestBody.prototype */ {
    ...
    update: function (options) {
        ...
        var rawMode = options.mode && options.mode.toString().toLowerCase(),
            mode = RequestBody.MODES[rawMode] || RequestBody.MODES.raw,
            raw = options.raw,
        ...
        if (!raw) {
            // if the provided body is a string or non compliant object, use it directly
            if ((options.mode !== mode) && (mode === RequestBody.MODES.raw)) {
                raw = options;
            }
            else {
                raw = undefined; // set to undefined explicitly so that .toJSON calls remove undefined raw bodies
            }
        }
        ...
        // @todo: Possibly use _.mergeDefined here?
