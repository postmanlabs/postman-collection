...
_.extend(RequestBody.prototype, /** @lends RequestBody.prototype */ {
    ...
    isEmpty: function () {
        var mode = this.mode,
            data;
        // If there's no Mode set, treat it as an empty body.
        if (!mode) {
            return true;
        }

        data = this[mode];
        // Data is a property list (urlencoded, formdata)
        if (PropertyList.isPropertyList(data)) {
            return !data.count();
        }
        // Data is a string (raw)
        if (_.isString(data)) {
            return !data.length;
        }
        // We don't understand this type, so let's just return whatever we have
        return !!data;
    }
});
