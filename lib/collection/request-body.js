...
_.assign(RequestBody.prototype, /** @lends RequestBody.prototype */ {
    ...
    update: function (options) {
        ...
        if (options.graphql) {
            graphql = {
                ...
                variables: typeof graphql.variables === OBJECT ?
                    JSON.stringify(graphql.variables) : graphql.variables
            };
        }
    ...
    toString: function () {
        ...
        if (this.mode === RequestBody.MODES.graphql && typeof this.graphql === OBJECT) {
            // @note avoid JSON.stringify here, otherwise we need to JSON.parse
            // `variables` first and then stringify the entire content.
            // So, we traverse the graphql properties and generate the
            // stringified content.
            graphql = this.graphql;
            content = [];

            GRAPHQL_PROPS.forEach(function (prop) {
                if (!(graphql.hasOwnProperty(prop) && typeof graphql[prop] === STRING)) {
                    return;
                }

                if (graphql[prop].charAt(0) === OPEN_CURLY_BRACE) {
                    return content.push('"' + prop + '":' + graphql[prop]);
                }

                content.push('"' + prop + '":"' + graphql[prop].replace(/"/g, '\\"') + '"');
            });

            return content.length ? '{' + content.join(',') + '}' : EMPTY;
        }
