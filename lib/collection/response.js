...
_.assign(Response.prototype, /** @lends Response.prototype */ {
    ...
    /**
     * Returns the durations of each request phase
     *
     * @param {Boolean} [beautify] - if true, get time in largest possible unit oterwise in milliseconds
     * @returns {Object}
     *
     * @note example object returned
     * {
     ...
     * }
     */
    timingPhases: function (beautify) {
        // return if timing information is not provided for the object
        if (!this.timings) {
            return;
        }
        ...
        var phases = {
            ...
            tcp: this.timings.connect - this.timings.socket,
        ...
        return beautify ? this.beautifyTime(phases) : phases;
    },
    ...
    beautifyTime: function (obj) {
        ...
        return _.reduce(obj, function (beautiful, value, key) {
            const nanoSeconds = Math.floor(value*1000);

            for (i in intervals) {
                var calc = Math.floor(nanoSeconds / intervals[i]);
                if (calc > 1) { 
                    beautiful[key] = `${calc} ${i}s`;
                    return beautiful;
                }
                else if (calc == 1) {
                    beautiful[key] = `${calc} ${i}`;
                    return beautiful;
                }
            }
