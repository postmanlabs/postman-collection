var util = require('../util'),
     ...
     */
    filenameMatchRegex = /filename[^;\n=]*=(['"])?(([^;\n]*)[.]([^.;\n]*)|[^.;\n]*)\1/,

    /**
     * @private
     * Regex to match filename* and it's value in content-disposition header
     */
    fileEncodingType = /filename\*=[^;\n=]*?"?([^"']*)''(.*)(")?/,
    ...
    /**
     * @private
     * Regex to esacpe all the hexadecimal number in the string
     */
     ...
     */
    NON_LATIN_REGEXP = /[^\x20-\x7e\xa0-\xff]/g,
    ...
    getLatin = function (val) {
        return String(val).replace(NON_LATIN_REGEXP, '?');
    },
    ...
    decodeFileName = function (encodedFileName, charset) {
        // to binary string
        var binary = encodedFileName.replace(HEX_ESCAPE_REPLACE_REGEXP, decodeHexcode),
            value;
        switch (charset) {
            case 'iso-8859-1':
    ...
    */
    ...
    parseContentDispositionHeader = function (dispositionHeader, contentType) {
        ...
        encodingTypeMatches = fileEncodingType.exec(dispositionHeader);
