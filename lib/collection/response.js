 ...
 * @property {Object} [timings] - timing information related to request and response in milliseconds
 * @property {Number} [timings.start] - timestamp of the start of the request (in Unix Epoch milliseconds)
 * @property {Number} [timings.socket] - when a socket is assigned to the request (relative to timings.start)
 * @property {Number} [timings.lookup] - when the host name is resolved (relative to timings.start)
 * @property {Number} [timings.connect] - when a socket connection is established (relative to timings.start)
 * @property {Number} [timings.response] -  when a response is received to this request (relative to timings.start)
 * @property {Number} [timings.end] - when the last bytes of the response are received (relative to timings.start)
 *
...
_.assign(Response.prototype, /** @lends Response.prototype */ {
    update: function (options) {
        ...
        _.mergeDefined(this, /** @lends Response.prototype */ {
            ...
            /**
             * Timing information related to request and response in milliseconds
             *
             * @type {Object}
             */
...
_.assign(Response.prototype, /** @lends Response.prototype */ {
     ...
     * }
     */
    timingPhases: function () {
        var timings = this.timings;

        // bail out if timing information is not provided for the object
        if (!timings) {
            return;
        }

        // @note Source of this calculation logic can be found at below link
        //  - https://github.com/postmanlabs/postman-request/blob/master/request.js
        return {
            wait: timings.socket,
            dns: timings.lookup - timings.socket,
            tcp: timings.connect - timings.socket,
...
_.assign(Response, /** @lends Response */ {
    ...
    createFromNode: function (response, cookies) {
        ...
        if (response.timingStart) {
            timings = _.merge({ start: response.timingStart }, response.timings);
        }
