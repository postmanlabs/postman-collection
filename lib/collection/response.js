...
_.assign(Response.prototype, /** @lends Response.prototype */ {
    ...
    /**
    * Extracts mime info and filename of the response content
    * A fallback of default filename is given, if filename is not present in header
    *
    * @note return object
    * {
    *   mimeType: sanitized mime type,
    *   charset: the normalized charset of the content-type headers
    *   format: format specific to the type returned
    *   name: the filename of the response from disposition header or a default file name
    *   ext:  file extension from disposition header or from sanitized content type
    *   filename: name + ext,
    *   defaultExtension: the default from sanitized content type
    * }
    * @returns {Object} - Return contentInfo of the response
    */
    contentInfo: function () {
        var contentInfoObj = contentInfo(this);
        if (!contentInfoObj.name) {
            contentInfoObj.name = DEFAULT_RESPONSE_FILENAME;
            contentInfoObj.ext = contentInfoObj.defaultExtension;
        }
        contentInfoObj.filename = contentInfoObj.name;
        contentInfoObj.ext && (contentInfoObj.filename += (DOT + contentInfoObj.ext));
