var util = require('../util'),
    ...
    MATCH_KNOWM_RESPONSE_ENCODING = /^(gzip|deflate)$/,
...
_.assignIn(Response.prototype, /** @lends Response.prototype */ {
    update: function (options) {
        _.assignIn(this, /** @lends Response.prototype */ {
            /**
             * @type {Request}
             */
...
_.assignIn(Response.prototype, /** @lends Response.prototype */ {
    ...
    mime: function (contentType, contentDisposition) {
        var self = this,
            detected = fileType(self.stream || self.body), // detect the mime from response body
            source = 'forced',
            mime;

        // if no overrides provided, we take the values from headers
        !contentDisposition && (contentDisposition = self.headers.one('content-disposition'));
        if (!contentType) {
            contentType = self.headers.one('content-type') && self.headers.one('content-type').value;
            source = 'header';
        }
    ...
    size: function () {
        ...
        if (contentLength && _.isString(contentEncoding) && contentEncoding.match(MATCH_KNOWM_RESPONSE_ENCODING) &&
                util.isNumeric(contentLength)) {
...
_.assignIn(Response, /** @lends Response */ {
    /**
