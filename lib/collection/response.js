...
_.extend(Response.prototype, /** @lends Response.prototype */ {
    ...
    size: function () {
        ...
        // else size of body is added
            ...
            sizeInfo.body = (this.stream && _.has(this.stream, 'byteLength')) ? (this.stream.byteLength || 0) :
                ((this.body != null) ? (supportsBuffer ? Buffer.byteLength(this.body.toString()) :
                    this.body.toString().length) : 0);
        }

        // size of header is added
        sizeInfo.header = Header.size(this.headers, this.code, this.reason());
        sizeInfo.total = (sizeInfo.header || 0) + (sizeInfo.body || 0);
        return sizeInfo;
    }
});
