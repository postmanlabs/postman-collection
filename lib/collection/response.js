var util = require('../util'),
    ...
    /**
     * @private
     * Regex to match filename and it's value in content-disposition header
     * Group 1 will be matched single or double quotes which can be enclosed within value of filename
     * Group 2 will be matched to the entire file name - basename and extension name
     * Group 3 will be matched to base name of file name
     * group 4 will be matched to extension name of file name
     */
    filenameMatchRegex = /filename[\x09\x20]*=[\x09\x20]*(")?(([^;\n]*)[.]([^.;\n]*)|[^.;\n]*)\1/,

    /**
     * @private
     * Regex to match filename* and it's value in content-disposition header
     * Group 1 will be matched to the character-set
     * Group 2 will be match to the encoded string
     */
    filenameEncodingMatchRegex = /filename\*[\x09\x20]*=[\x09\x20]*([^"']*)'[^;\n=]*?'(.*)/,
    ...
    */
    parseContentDispositionHeader = function (dispositionHeader, contentType) {
        ...
        encodingTypeMatches = filenameEncodingMatchRegex.exec(dispositionHeader);
        if (encodingTypeMatches && encodingTypeMatches[1]) {
            decodedFileName = decodeFileName(encodingTypeMatches[2], encodingTypeMatches[1]);

            // Returns default file name, if charset is not supported
            if (!decodedFileName) {
                return defaultFileNameObj;
            }
            matches = fileNameSplitRegex.exec(decodedFileName);
        ...
        matches = filenameMatchRegex.exec(dispositionHeader);

        // Returns default file name if filename is empty
        if (!matches || !matches[2] && !matches[3]) {
            return defaultFileNameObj;
        }
        // Returns empty extension if filename doesn't have extension
        if (!matches[3]) {
            return {
                name: matches[2],
                ext: E
