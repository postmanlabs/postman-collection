...
_.assign(Request.prototype, /** @lends Request.prototype */ {
    ...
    toJSON: function () {
        ...
        // if request url lacks variables, then export the same as string
        // @todo this was already present in v2, hence did not change behaviour.
        // @todo this has potential data loss of if query string or other stuff has descriptions
        if (obj.url && !_.isString(obj.url) && !(this.url.variables && this.url.variables.count())) {
     ...
     */
    authorize: function () {
        throw new Error('collection request.authrozie() has been discontinued');
    }
});
