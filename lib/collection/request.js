...
_.assign(Request.prototype, /** @lends Request.prototype */ {
    ...
    size: function () {
        var contentLength = this.headers.get(CONTENT_LENGTH),
            ...
            bodyString = _.isFunction(this.body && this.body.toString) && this.body.toString(),
            sizeInfo = {
                body: 0,
                header: 0,
                total: 0
        ...
        if (contentLength && util.isNumeric(contentLength)) {
            sizeInfo.body = parseInt(contentLength);
        }
        ...
        sizeInfo.header = (this.method + SP + requestTarget + SP + HTTP_X_X + CRLF).length + this.headers.contentSize();
