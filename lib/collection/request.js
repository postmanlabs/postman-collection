...
_.assign(Request.prototype, /** @lends Request.prototype */ {
    ...
    upsertHeader: function (header) {
        ...
        var self = this,
            ...
            // flag to indicate if incoming header value has multiple values
            multiValue = Array.isArray(header.value),

            // existing header(first match), if present
            existing = this.headers.find({ key: header.key }),

            // flag to indicate whether to insert a new header record or not
            insert = !existing,

            // reference id used to store the existing header
            referenceId = existing && (self.headers._postman_listIndexCaseInsensitive ?
                existing.key.toLowerCase() : existing.key);

        // if there's an existing header with multiValue or the given input have multiValue,
        // it's better to remove the existing header and and insert new records.
        // since it's difficult to correctly update the members and references of
        // a HeaderList due to multiValue and case insensitive names.
        // side effects:
        //   1. loses header index
        //   2. probably expensive (benchmark this vs finding individual reference)
        // @todo FIX IT ¯\_(ツ)_/¯
        if (referenceId && (Array.isArray(self.headers.reference[referenceId]) || multiValue)) {
            self.removeHeader(header.key);
            insert = true; // since the existing header is removed
        }

        // if it's a new header with multiple values, create separate entry for each key-value[i]
        if (insert && multiValue) {
            header.value.forEach(function (value) {
                self.headers.add({ key: header.key, value: value });
            });
            return;
        }

        // if it's a new header with single value, create a new header record
        if (insert) {
            self.headers.add(header);
            return;
        }

        // at this point we know that the existing header and given header are
        // single-valued, so we can directly update
