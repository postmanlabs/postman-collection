...
_.assign(PropertyList.prototype, /** @lends PropertyList.prototype */ {
    ...
    /**
     * Returns the JSON representation of a property list
     *
     * @param {?Object} [options={}] - A set of options
     * @param {?Boolean} [options.excludeDisabled=false] - Set to true to exclude disabled properties from the resultant
     * @return {Object}
     */
    toJSON: function (options) {
        ...
        return _.reduce(this.members, function (result, member) {
            // use member.toJSON if it exists
            if (!_.isEmpty(member) && _.isFunction(member.toJSON)) {
                if (options.excludeDisabled ? !member.disabled : true) {
                    result.push(member.toJSON(options));
                }
                return result;
            }

            json = _.reduce(member, function (accumulator, value, key) {
                ...
                if (value && _.isFunction(value.toJSON)) {
                    accumulator[key] = value.toJSON(options);
                    return accumulator;
                }
