...
_.inherit((
    ...
    PropertyList = function PostmanPropertyList (type, parent, populate) {
        ...
        _.assignIn(this, /** @lends PropertyList.prototype */ {
            /**
             * @private
             * @type {Array}
             */
...
_.assignIn(PropertyList.prototype, /** @lends PropertyList.prototype */ {
    ...
    remove: function (predicate, context) {
        ...
        if (_.isString(predicate)) {
            (match = list.one(predicate)) && (predicate = function (item) {
                return (item === match);
            });
        }
        // in case an object reference is sent, prepare it for removal
        else if (predicate instanceof list.Type) {
            match = predicate;
        ...
        _.isFunction(predicate) && _.remove(list.members, function (item) {
            var index;
            if (predicate.apply(context, arguments)) {
                if ((index = item[list._postman_propertyIndexKey]) && (index = String(index))) {
                    list._postman_propertyIndexCaseInsensitive && (index = index.toLowerCase());
                    delete list.reference[index];
                }
    ...
    each: function (iterator, context) {
        _.forEach(this.members, iterator.bind(context || this));
    },
    ...
    filter: function (iterator, context) {
        return _.filter(this.members, iterator.bind(context || this));
    },
    ...
    find: function (iterator, context) {
        if (_.isFunction(iterator)) {
            iterator = iterator.bind(context || this);
        }

        return _.find(this.members, iterator);
    },
    ...
    map: function (iterator, context) {
        return _.map(this.members, iterator.bind(context || this));
    },
     ...
     */
    has: function (item) {
        return Boolean(_.isString(item) ? this.one(item) : this.filter(function (member) {
            return member === item;
        }).length);
    },
...
_.assignIn(PropertyList, /** @lends PropertyList */ {
    /**
