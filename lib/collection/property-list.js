...
_.assign(PropertyList.prototype, /** @lends PropertyList.prototype */ {
     ...
     *   1. if used with a `customizer`, the `customizer` is invoked from the last value,
     *      and the first value for which the `customizer` returns `true` is returned.
     *   2. the last value is returned.
     *
     * @param {String} id
     ...
     */
    one: function (id, customizer) {
        var val = this.reference[this._postman_listIndexCaseInsensitive ? String(id).toLowerCase() : id],
            i;

        // return the first match if duplicates are not allowed
        if (!(this._postman_listAllowsMultipleValues && Array.isArray(val))) {
            return val;
        }

        i = val.length && (val.length - 1);

        if (!i) {
            return undefined;
        }

        // if there are duplicates, and there is no customizer, return the last value
        if (!customizer) {
            return val[i];
        }

        // invoke customizer from the last value
        customizer = customizer.bind(this);

        while (i >= 0) {
            // return the value if customizer returns true
            if (customizer(val[i])) { return val[i]; }

            i--;
        }

        // customizer did not return truthy for any match
        return undefined;
    },
