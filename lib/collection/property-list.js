...
_.assign(PropertyList.prototype, /** @lends PropertyList.prototype */ {
    ...
    add: function (item) {
        ...
        return this.insert((item.constructor === this.Type) ? item :
    ...
    /**
     * Updates or inserts given item in the list. Existence is checked by using the correct index key.
     *
     * @param {Object} item - Item to update or insert.
     */
    upsert: function (item) {
        var rule = {},
            existing;

        rule[this._postman_listIndexKey] = item[this._postman_listIndexKey];
        existing = this.find(rule);

        if (!existing) {
            return this.add(item);
        }

        // Create a new item from args, to ensure that any processing of properties takes place
        item = (_.has(this.Type, 'create') ? this.Type.create.apply(this.Type, item) : new this.Type(item));

        _.replaceProps(existing, item);
        return existing;
    },
