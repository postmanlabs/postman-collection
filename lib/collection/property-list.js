...
_.assign(PropertyList.prototype, /** @lends PropertyList.prototype */ {
    ...
    /**
     * Get Item in this list by `ID` reference. If multiple values are allowed,
     * the last enabled value is returned.
     *
     * @param {String} id
     * @returns {PropertyList.Type}
     */
    lastEnabled: function (id) {
        var i,
            reference = this.reference[this._postman_listIndexCaseInsensitive ? String(id).toLowerCase() : id];

        // bail out if no reference found with the given identifier
        if (!reference) {
            return;
        }

        // if reference is single-valued, directly return the reference if not disabled
        if (!Array.isArray(reference)) {
            return reference.disabled ? undefined : reference;
        }

        // else, traverse the reference list in reverse direction in order to
        // find the last enabled.
        for (i = reference.length - 1; i >= 0; i--) {
            if (reference[i] && !reference[i].disabled) {
                return reference[i];
            }
        }
    },
    ...
    /**
     * Removes an element from the PropertyList for which the which the `ID` matches.
     * If multiple values are allowed, the last enabled value is removed.
     *
     * @param {String} id
     */
    removeLastEnabled: function (id) {
        var i,
            j,
            referenceId = this._postman_listIndexCaseInsensitive ? String(id).toLowerCase() : id,
            reference = this.reference[referenceId];

        // bail out if no reference found for the given `ID`
        if (!reference) {
            return;
        }

        // if the Property has a multiValue reference, remove a single element
        // from the reference and member list.
        // @note: Unlike PropertyList~remove, since a single element is needed to be removed.
        // so instead of traversing the members, reverse lookup on PropertyList reference.
        if (Array.isArray(reference)) {
            // traverse the references in reverse direction in order to find the
            // last enabled Property we are looking for
            for (i = reference.length - 1; i >= 0; i--) {
                // continue searching until we find the matching enabled Property
                if (!(reference[i][this._postman_listIndexKey] === id && !reference[i].disabled)) {
                    continue;
                }

                // @note: at this point, we know that the given PropertyList has the
                // required enabled Property we are looking for
                // first: remove the Property from the references list
                reference.splice(i, 1);

                // if the array becomes empty, remove it
                (reference.length === 0) && (delete this.reference[referenceId]);

                // if the array contains only one element, remove the array, and
                // assign the element as the reference value
                (reference.length === 1) && (this.reference[referenceId] = reference[0]);

                // second: remove the Property from the members list as well
                // @note: traverse the members in reverse direction to make sure
                // only the last enabled Property will be removed.
                for (j = this.members.length - 1; j >= 0; j--) {
                    if (!(this.members[j][this._postman_listIndexKey] === id && !this.members[j].disabled)) {
                        continue;
                    }

                    // finally: remove the Property from the members list
                    // and stop searching further.
                    this.members.splice(j, 1);
                    break;
                }

                // break, since Property is removed from both reference and members
                break;
            }
        }
        // else, directly remove the single-valued Property which is not disabled
        // otherwise, do not remove disabled Property.
        else if (!reference.disabled) {
            // delete Property reference
            delete this.reference[referenceId];

            // this traversal can be done in a forward direction as there are not duplicates.
            for (j = this.members.length - 1; j >= 0; j--) {
                if (this.members[j][this._postman_listIndexKey] !== id) {
                    continue;
                }

                // remove the single element and stop searching further.
                this.members.splice(j, 1);
                break;
            }
        }
