...
_.extend(Url.prototype, /** @lends Url.prototype */ {
    ...
    update: function (url) {
        ...
        _.isString(path) && (path = _.trim(path, '/').split('/'));
        // expand host string
        _.isString(host) && (host = _.trim(host, '.').split('.'));
    ...
    /**
     * The plain Javascript representation of the URL
     * @returns {{protocol: *, auth: *, host: *, port: *, path: (*|string), query: (*|Array), hash: *}}
     */
    toJSON: function () {
        return {
            protocol: this.protocol,
            auth: _.clone(this.auth),
            host: this.getHost(),
            port: this.port,
            path: this.getPath({ unresolved: true }),
            query: QueryParam.parse(this.getQueryString()),
            hash: this.hash,
