...
_.assign(Url.prototype, /** @lends Url.prototype */ {
     ...
     *
     * @param {?Booleamn=} [unresolved=false]
     * @returns {string}
     *
     ...
     *
     * @param {?Boolean} [encoded=false] - Enables URL encoding when processing the query string
     * @returns {String}
     *
     ...
     */
    getQueryString: function (encoded) {
        if (!this.query.count()) {
            return E;
        }

        if (typeof encoded === 'object') { // @todo discontinue in v4
            return QueryParam.unparse(this.query.all(), encoded);
        }

        return QueryParam.unparse(this.query.all(), { encode: encoded });
    },
