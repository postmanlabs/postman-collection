...
_.assign(Url.prototype, /** @lends Url.prototype */ {
     ...
     * @returns {string}
     */
    toEncodedString: function (forceProtocol) {
        var rawUrl = E,
            protocol = this.protocol;
        ...
        if (this.path) {
            rawUrl += this.getPath(false, true);
        }

        if (this.query && this.query.count()) {
            rawUrl += QUERY_SEPARATOR + this.getQueryString({ ignoreDisabled: true, encode: true });
        }
     ...
     */
    getPath: function (unresolved, encode) {
        if (typeof unresolved === OBJECT) { // @todo discontinue in v4
            unresolved = unresolved.unresolved;
            encode = unresolved.encode;
        }

        // only encode if unresolved === false
        encode = !unresolved && encode || false;
        ...
        segments = _.transform(this.path, function (res, segment) {
            ...
            resolvedSegment = _.isString(variable) ? variable : segment;
            resolvedSegment = encode && urlEncoder.encodeString(resolvedSegment) || resolvedSegment;
            res.push(resolvedSegment);
        }, []);
     ...
     * @param {Boolean} [encodePath=false] - when set to `true` the path will be URL encoded
     * @returns {*|string}
     * @example /something/postman?hi=notbye
     */
    getPathWithQuery: function (encodeQuery, encodePath) {
        var path = this.getPath(false, encodePath);
     ...
     *
     * @param {Boolean} [encode] - punycode encode host if set to true
     * @returns {string}
     */
    getHost: function (encode) {
        if (!this.host) {
            return E;
        }

        var host = _.isArray(this.host) ? this.host.join(DOMAIN_SEPARATOR) : this.host.toString();

        return encode && urlEncoder.encodeHostName(host) || host;
    },
     ...
     * @param {Boolean} [encode=false] - use encoded host if set to true
     * @returns {String}
     *
     ...
     */
    getRemote: function (forcePort, encode) {
