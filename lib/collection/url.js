...
_.assign(Url, /** @lends Url */ {
    ...
    parse: function (url) {
        ...
        _.isString(url) && (query = _.clone(url)) && (temp = query.split('?')[1]) && (keyValue = temp.split('&'));

        keyValue && keyValue.forEach(function (kv) {
            var rgx = new RegExp('{{([^]*?)}}', 'g'),
                split;

            if (rgx.test(kv)) {
                split = kv.split('=');
                query = query.replace(kv, (split[0] + '=' + undefined));
                result.push(kv);
            }
        });

        p.query = _.get(query.match(regexes.extractQuery), GET_1);

        result.forEach(function (kv) {
            var k = kv.split('=');

            p.query = p.query.replace((k[0] + '=' + undefined), kv);
        });
