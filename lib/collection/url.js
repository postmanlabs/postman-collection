...
_.assign(Url, /** @lends Url */ {
    ...
    parse: function (url) {
        ...
        if (_.isString(p.protocol)) {
            url = url.substr(p.protocol.length); // remove that damn protocol from url

            // remove the scheme host split (://)
            schemeSeparator = url.match(regexes.schemeSplit);
            schemeSeparator && (url = url.substr(_.get(schemeSeparator, [GET_1, 'length'], 3)));
        }
        ...
        if (_.isString(p.host)) {
            url = url.substr(p.host.length + url.indexOf(p.host)); // remove host from url
