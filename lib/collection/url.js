...
_.assign(Url, /** @lends Url */ {
    ...
    parse: function (url) {
        ...
        _.isString(url) && (keyValue = url.match(regexes.extractVariables));

        keyValue && keyValue.forEach(function (val) {
            if (regexes.reservedCharsVariables.test(val)) {
                var identifier = uuid();
                url = url.replace(val, identifier);
                result.push({ id: identifier, value: val });
            }
        });
        ...
        if (_.isString(p.host)) {
            ...
            result.forEach(function (kv) {
                p.host = p.host.replace(kv.id, kv.value);
            });
        ...
        if (_.isString(p.path)) {
            ...
            result.forEach(function (kv) {
                p.path = p.path.replace(kv.id, kv.value);
            });
        ...
        _.isString(p.query) && result.forEach(function (kv) {
            p.query = p.query.replace(kv.id, kv.value);
        });
        ...
        _.isString(p.hash) && result.forEach(function (kv) {
            p.hash = p.hash.replace(kv.id, kv.value);
        });
