...
_.assignIn(Url.prototype, /** @lends Url.prototype */ {
    /**
     * Set a URL
     * @draft
     *
    ...
    toString: function (options) {
        ...
        if (this.protocol) {
            rawUrl += (_.endsWith(this.protocol, '://') ? this.protocol : this.protocol + '://');
        }

        if (this.auth && this.auth.user) { // If the user is not specified, ignore the password.
            rawUrl += ((this.auth.password) ?
        ...
        if (this.port) {
            rawUrl += (':' + this.port.toString(options));
        }

        if (this.path) {
            rawUrl += this.getPath(options);
        }

        if (this.query && this.query.count()) {
            rawUrl += ('?' + this.getQueryString(options));
        }
    ...
    getQueryString: function (options) {
        ...
        if (self.query.count()) {
            return QueryParam.unparse(self.query.all(), options);
        }
        return '';
    },
