...
_.extend(Url.prototype, /** @lends Url.prototype */ {
    ...
    update: function (url) {
        ...
        var parsedUrl = _.isString(url) ? Url.parse(url) : url,
            ...
            variable = parsedUrl.variables || parsedUrl.variable;
    ...
    toString: function () {
        ...
        if (this.protocol) {
            rawUrl = rawUrl + (_.endsWith(this.protocol, PROTOCOL_SEPARATOR) ?
                this.protocol : this.protocol + PROTOCOL_SEPARATOR);
        }
        ...
        if (this.port) {
            rawUrl = rawUrl + PORT_SEPARATOR + this.port.toString();
        }
        ...
        if (this.query && this.query.count()) {
            rawUrl = rawUrl + QUERY_SEPARATOR + this.getQueryString();
        }

        if (this.hash) {
            rawUrl = rawUrl + SEARCH_SEPARATOR + this.hash;
        }
