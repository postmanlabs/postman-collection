...
_.assign(ItemGroup.prototype, /** @lends ItemGroup.prototype */ {
    ...
    /**
     * Set, update or delete protocol profile behavior for the current ItemGroup.
     *
     * @example <caption> Set or update protocol profile behavior </caption>
     * item.setProtocolProfileBehavior({ key: 'strictSSL', value: false });
     *
     * @example <caption> Set protocol profile behavior and remove that from children Item or ItemGroup </caption>
     * // remove same behavior from children to force current behavior on them.
     * item.setProtocolProfileBehavior({ key: 'strictSSL', value: false, removeFromChildren: true });
     *
     * @example <caption> Delete protocol profile behavior </caption>
     * // missing property value signifies delete
     * item.setProtocolProfileBehavior({ key: 'strictSSL' });
     *
     * @private
     * @param {Object} obj
     * @param {String} obj.key
     * @param {?*} obj.value
     * @param {?Boolean} obj.removeFromChildren
     * @returns {ItemGroup}
     */
    setProtocolProfileBehavior: function (obj) {
        // update behavior for current ItemGroup
        Item.prototype.setProtocolProfileBehavior.call(this, obj);

        // bail out if removeFromChildren is not set
        if (!(obj && obj.key && obj.removeFromChildren)) { return this; }

        // remove behavior from each Item or ItemGroup
        this.items.each(function (item) {
            // missing property value deletes the behavior if set already
            Item.prototype.setProtocolProfileBehavior.call(item, { key: obj.key });
        }, this);
