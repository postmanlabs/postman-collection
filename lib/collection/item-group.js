...
_.assign(ItemGroup.prototype, /** @lends ItemGroup.prototype */ {
     ...
     */
    one: function (idOrName) {
        if (!_.isString(idOrName)) { return; }

        var item,
            predicate;

        predicate = function (eachItem) {
            ...
            if (ItemGroup.isItemGroup(eachItem)) {
                item = eachItem.one(idOrName);
                return !Boolean(item); // bail out of the for loop if we found anything
            }
        };

        this.items.each(predicate);
