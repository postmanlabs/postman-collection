...
_.assign(ProxyConfig.prototype, /** @lends ProxyConfig.prototype */ {
    ...
    updateProtocols: function (protocols) {
        ...
        var updatedProtocols,
            updatedMatch,
            hostAndPath = this.match.pattern.split(PROTOCOL_HOST_SEPARATOR)[1];

        if (!hostAndPath) {
            return;
        }

        _.isArray(protocols) && (updatedProtocols = _.intersection(AllOWED_PROTOCOLS, protocols));
        _.isString(protocols) && (updatedProtocols = _.intersection(AllOWED_PROTOCOLS, [protocols]));

        updatedProtocols = _.isEmpty(updatedProtocols) ? AllOWED_PROTOCOLS : updatedProtocols;
        updatedMatch = {
            pattern: updatedProtocols.join(PROTOCOL_DELIMITER) + PROTOCOL_HOST_SEPARATOR + hostAndPath
        };
        this.match.update(updatedMatch);
    },
    ...
    getProxyUrl: function (urlStr) {
        ...
        return (url.protocol || DEFAULT_PROTOCOL) + PROTOCOL_HOST_SEPARATOR + this.host + COLON + this.port;
    },
