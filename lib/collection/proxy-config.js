...
_.assign(ProxyConfig.prototype, /** @lends ProxyConfig.prototype */ {
    ...
    getProxyUrl: function () {
        var proxyAuth = '',
            auth = this.auth && this.auth.type === BASIC_AUTH_METHOD && this.auth.current();

        auth && (proxyAuth = `${auth.username}:${auth.password}@`);

        return DEFAULT_PROTOCOL + PROTOCOL_HOST_SEPARATOR + proxyAuth + this.host + COLON + this.port;
    },
