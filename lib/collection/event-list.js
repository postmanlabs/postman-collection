...
_.assign(EventList.prototype, /** @lends EventList.prototype */ {
     ...
     *
     * @param {String} name
     * @param {?Boolean} excludeDisabled - When set to true, disabled events are excluded from the resultant
     * @returns {Array<Event>}
     */
    listeners: function (name, excludeDisabled) {
        var all;

        // we first procure all matching events from this list
        all = this.listenersOwn(name, excludeDisabled);
        ...
        this.eachParent(function (parent) {
            ...
            (parent !== this.__parent) && EventList.isEventList(parent.events) &&
                (parentEvents = parent.events.listenersOwn(name, excludeDisabled)) && parentEvents.length &&
     ...
     *
     * @param {string} name
     * @param {?Boolean} excludeDisabled - When set to true, disabled properties are excluded from the resultant
     * @returns {Array<Event>}
     */
    listenersOwn: function (name, excludeDisabled) {
        return this.filter(function (event) {
            return (event.listen === name && (excludeDisabled ? !event.disabled : true));
        });
    }
});
